...
router.get('/list', async (req, res) => {
    ...
    // res.json(output);
    res.render('cs_messenger/list', output);
});
...
async function getListData(req,res){
    ...
    const t_sql = `SELECT COUNT(1) totalRows FROM cs_messenger ${where}`;
    ...
    if (totalRows > 0) {
        ...
        const sql = `SELECT * FROM \`cs_messenger\` ${where} ORDER BY cs_messenger.sid DESC LIMIT ${(page-1)*perPage}, ${perPage}`;
...
router.get('/api/list', async (req, res)=>{
    ...
    .post(async (req, res) => {
        ...
        const sql = "INSERT INTO `cs_messenger` SET ?"
        let result = {};
        // 處理新增資料時可能的錯誤
        try {
    ...
    router.delete('/delete/:sid([0-9]+)', async (req, res) => {
        ...
        const sql = "DELETE FROM cs_messenger WHERE sid=?";
    ...
    router.get("/edit", async (req, res) => {
        // let courseSid = ;
        const sql = `SELECT * FROM \`cs_messenger\` WHERE sid=?`;
      ...
      router.post("/edit", async (req, res) => {
        ...
        // console.log(input);
        const sql = `UPDATE \`cs_messenger\` SET ? WHERE sid=?`;
        let result = {};
        // 處理修改資料時可能的錯誤
        try {
